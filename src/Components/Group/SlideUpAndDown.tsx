...
const SlideUpAndDown = ({RegionArray, DetailData,Curr_lan, Curr_log}:Props) => {
    ...
    for (let i:number = 0; i <RegionArray.length; i= i+1) {
      boxList.push(
          <RegionComp key={i} icon={RegionArray[i].icon} lan={RegionArray[i].latitude} log={RegionArray[i].longitude} Name={RegionArray[i].Title} CallNo={RegionArray[i].CallNo} Adress={RegionArray[i].Adress} 
